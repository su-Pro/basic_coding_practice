...
# minDistance, maxDistance] where minDistance is the minimum distance between any two 
# distinct critical points and maxDistance is the maximum distance between any two 
...
class Solution:
    def nodesBetweenCriticalPoints(self, head: Optional[ListNode]) -> List[int]:
        ...
        while cur.next is not None:
            post = cur.next
            if cur.val > prev.val and cur.val > post.val or cur.val < prev.val and cur.val < post.val:
                idxs.append(i)
            prev = cur
            cur = cur.next
            i += 1
        ...
        for i in range(1, len(idxs)):
            v_min = min(idxs[i] - idxs[i - 1], v_min)
        return [v_min, idxs[-1] - idxs[0]]
